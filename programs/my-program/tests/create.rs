...
use my_program::{instruction::CreateArgs, state::MyAccount};
...
#[tokio::test]
async fn create() {
    let mut context = ProgramTest::new("my_program", my_program::ID, None)
        .start_with_context()
        .await;
    ...
    let ix = my_program::instruction::create(
        &address.pubkey(),
